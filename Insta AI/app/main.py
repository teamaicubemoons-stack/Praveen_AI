...
app = FastAPI(
    title="Insta AI Automation",
    description="AI-powered Instagram DM automation backend",
    version="1.0.0"
)

# Include Routers
app.include_router(webhook.router, prefix="/api/v1", tags=["Instagram Webhook"])
...
async def startup_event():
    ...
    """
    logger.info("="*60)
    logger.info("🚀 INITIALIZING INSTA AI BACKEND SYSTEM")
    logger.info("="*60)
    
    # 1. Verify Meta / Instagram Config
    meta_status = "✅ LOADED" if all([settings.INSTAGRAM_BUSINESS_ACCOUNT_ID, settings.PAGE_ACCESS_TOKEN]) else "❌ INCOMPLETE"
    logger.info(f"MODULE: Meta/Instagram Configuration -> {meta_status}")
    
    if settings.INSTAGRAM_BUSINESS_ACCOUNT_ID:
        logger.info(f"   - Business ID: {settings.INSTAGRAM_BUSINESS_ACCOUNT_ID}")
    else:
        logger.error("   - Business ID: MISSING (Check .env)")
...
async def root():
    return {
        "message": "Insta AI Backend is running",
        "docs": "/docs",
        "status": "healthy"
    }
